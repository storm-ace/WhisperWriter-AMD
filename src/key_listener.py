...
class KeyListener:
    ...
    def __init__(self):
        ...
        self.key_chord = None
        self.key_chord_secondary = None
        # Records which key triggered the most recent activation: True when the
        # secondary ("no Enter") key fired, False for the primary key.
    ...
    def load_activation_keys(self):
        ...
        The primary key follows the `press_enter` setting; the optional
        secondary key always dictates without pressing Enter.
        """
        ...
        secondary = ConfigManager.get_config_value('recording_options', 'activation_key_no_enter')
        secondary_keys = self.parse_key_combination(secondary) if secondary else set()
        self.key_chord_secondary = KeyChord(secondary_keys) if secondary_keys else None
    ...
    def on_input_event(self, event):
        """Handle input events and trigger callbacks if a key chord becomes active or inactive.

        Both the primary and the optional secondary chord are fed every event;
        each tracks only its own keys. On activation, `suppress_enter` records
        which chord fired so the result handler can decide whether to press Enter.
        """
        ...
        for chord, suppress in ((self.key_chord, False), (self.key_chord_secondary, True)):
            if not chord:
                continue
