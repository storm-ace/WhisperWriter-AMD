...
class InputSimulator:
    ...
    def press_enter(self):
        """
        Press Enter after the text was inserted, e.g. to submit a chat message.

        Works regardless of input_method: reuses the pynput controller when one
        exists (pynput/clipboard), otherwise creates one on the spot.
        """
    ...
    def _paste_clipboard(self, text):
        """
        Insert text via the system clipboard and a single Ctrl+V.

        Instant and lossless — unlike per-character typing, no keystrokes
        (notably spaces) can be dropped by a busy target window. The previous
        clipboard contents are restored afterwards, unless the
        'restore_clipboard' option is disabled — then the dictated text stays
        on the clipboard so it can be pasted manually if the automatic Ctrl+V
        failed.
        """
        restore = ConfigManager.get_config_value('post_processing', 'restore_clipboard')
        ...
        pyperclip.copy(text)
        time.sleep(0.05)  # let the OS register the new clipboard before pasting
